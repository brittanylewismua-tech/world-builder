...
function StudioBody({ world }: { world: World }) {
  ...
  */
  ...
  async function publishNow() {
    ...
    const consequences = [
      `Drop ${String(drop.number).padStart(2, "0")} moves into Drop History, where you can open it any time — and re-open it to work on if you want it back.`,
      next
          ...
          */}
          <span className="flex items-center gap-1.5">
            ...
            <Explain label="What does finishing early do?">
              Moves this drop into Drop History and starts the next one now,
              instead of waiting for its date. Your research board comes with
              you, and Drop History can re-open it. Nothing is sent anywhere —
