...
/* ------------------------------------------------------------------ */
/* handing it to Goldie                                                */
/* ------------------------------------------------------------------ */

/**
 * The whole group as one block of text.
 *
 * Image links are no use to a chat that cannot open them, and most cannot.
 * What travels is words — the patterns already written, plus Etsy's own
 * description of each design, plus the two numbers that mean something. It
 * pastes into anything and costs nothing to produce.
 */
export function briefing(
  ...
  const lines: string[] = [
    `Top sellers on Etsy for "${keyword}" — part of my ${worldName} world.`,
    "",
  ];

  if (brief?.patterns.length) {
    lines.push("WHAT IS WORKING", "");
    for (const p of brief.patterns) lines.push(`${p.heading}. ${p.body}`, "");
  }

  lines.push("THE DESIGNS", "");
  for (const w of [...list].sort((a, b) => b.sales - a.sales)) {
    const saved = w.views
      ? ` · saved by ${((100 * w.hearts) / w.views).toFixed(1)}% of the people who saw it`
      : "";
    lines.push(
      `${w.design || w.title}`,
      `${w.sales.toLocaleString()} sales over ${w.ageDays} days${saved}`,
      w.url,
      "",
    );
  }
...
/** The designs themselves, as an archive to drag into a chat that can look
 *  at pictures. The bytes come from the server because a browser cannot save
 *  a cross-origin image and Etsy's CDN will not hand them to this page. */
export async function downloadDesigns(world: World, keyword: string) {
  const { data } = await supabase.auth.getSession();
  const token = data.session?.access_token;
  if (!token) throw new Error("You are signed out. Reload and sign back in.");

  const r = await fetch("/api/winners/designs", {
    method: "POST",
    headers: {
      "content-type": "application/json",
      authorization: `Bearer ${token}`,
    },
    body: JSON.stringify({ worldId: world.id, keyword }),
  });

  if (!r.ok) {
    let msg = "That download did not go through.";
    try {
      msg = (await r.json()).error ?? msg;
    } catch {
      /* a non-JSON failure keeps the plain message */
    }
    throw new Error(msg);
  }
  ...
  a.href = url;
  a.download = `${keyword} designs.zip`;
  a.click();
